...
//                        .reduce((x, y) -> (x.getTotalAmount() >= y.getTotalAmount() ? x : y)).orElseThrow(NullPointerException::new).getCustomerName(); // 단건을 의미하는 경우

        String highestOrderCustomer = totalAmountByCustomer
                .entrySet().stream()
